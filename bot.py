...
BODY = {
    ...
    "thigh_len":  0.59,   # upper leg: 45 cm
    "shin_len":   0.50,   # lower leg: 38 cm
...
def my_fitness(data):
    ...
    # #
    if data["falls"] > 0: 
        return -20.0 * data["falls"] + data["distance"] * 5
    ...
    score += data["legs_active"] *  0.12
    avg_height = data.get("height_total", 0) / 600
    # score*=(1-data.get("total_lean"))
    avg_speed=data.get("forward_vel")/600
    if avg_speed >5:
        score+=data["smoothness"]*100
        score += (avg_height * 0.1)
  

    score -= data.get("backward_frames", 0) * 0.15
    score += data.get("air_frames", 0) * 0.05
    # score+=data.get("alternate_legs")*0.20*data.get("total_lean")    // to ensure ki both legs contribute equally in movement 
    
    score+=data.get("sprint_frames",0)*1
    ...
    # # ── PART B: Reward upright walking ────────────────────────────────
    # # 1. Calculate clean, frame-independent averages
    
    # avg_lean = data.get("total_lean", 0) / 600
   
    # avg_height = data.get("height_total", 0) / 600

    # # 2. THE MULTIPLIER CORE:
    # # Instead of giving flat addition points for walking nicely, we use them to 
    # # increase the VALUE of each meter traveled. 
    # # Base multiplier starts at 12.0. If they alternate legs and stay upright, it grows!
    
    # distance_multiplier = 12.0
    
    # # Reward using both legs symmetrically (max +3.0 to multiplier)
    # distance_multiplier += data["legs_active"] * 3.0
    
    # # Reward alternating left-right stances (max +4.0 to multiplier)
    # # 500 single-leg frames / 600 = ~0.83 max value. 0.83 * 5.0 = +4.1
    # distance_multiplier += (data.get("stepping_frames", 0) / 600) * 5.0
    
    # # Penalize heavy leaning by lowering the multiplier directly
    # # If they lean too much, their meters become worth way less points
    # distance_multiplier -= (avg_lean * 4.0)

    # # 3. CALCULATE CORE SCORE
    # # This guarantees that if distance is 0, the core score is exactly 0.
    # # Higher distance + better gait = explosive fitness growth.
    # score = data["distance"] * distance_multiplier

    # # 4. Small static adjustments (Kept minor so they never dominate distance)
    # score += data["step_count"] * 0.05
    # score += data["smoothness"] * 5.0
    # score+=avg_height*0.1
    # score -= data.get("backward_frames", 0) * 0.15
    # score += data.get("sprint_frames", 0) * 0.5
    # # score+=data.get("sprint_frames",0)*10
...
def my_metrics(step_data):
    return {
        ...
        "height_total": step_data["torso_height"],

        "forward_vel": max(0, step_data["vx"]),
        
        # Track if the bot is 'sprinting' (moving faster than 2 px/s)
        "sprint_frames": 1 if step_data["vx"] > 2 else 0,

        "speed": step_data["vx"],

        # "alternate_legs": 1 if step_data["feet_down"]==1 else 0,

        "total_lean":abs(step_data["torso_angle"])
